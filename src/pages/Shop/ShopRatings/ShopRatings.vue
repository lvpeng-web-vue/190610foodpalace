    ...
    <div class="ratings-content">
      ...
      <div class="ratingselect">
        <div class="rating-type border-1px">
          <span class="block positive" :style="{color: selectType==2?'green':'gray'}" @click="setSelectType(2)">
            全部
            <span class="count">{{ratings.length}}</span>
          </span>
          <span class="block positive" :style="{color: selectType==0?'green':'gray'}" @click="setSelectType(0)">
            满意
            <span class="count">{{positiveCount}}</span>
          </span>
          <span class="block negative" :style="{color: selectType==1?'green':'gray'}" @click="setSelectType(1)">
            不满意
            <span class="count">{{ratings.length-positiveCount}}</span>
          </span>
        </div>
        <div class="switch" :style="{color: onlyContent?'green':'gray'}" @click="toggleOnlyContent">
...
*/
        if (selectType === 2) {
          ...
          return (
            selectType === rateType && (!onlyContent || rating.text.length > 0)
          );
        }
      });
    }
